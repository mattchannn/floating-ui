...
import {useFloating, size, offset, detectOverflow, flip, autoUpdate} from '../';
...
export const Select: React.FC<{
  ...
  const {getReferenceProps, getFloatingProps} = useInteractions([
    useClick(context),
    useAria(context, {role: 'listbox'}),
    useDismiss(context),
    useFocusTrap(context, {inert: true, modal: false, scrollLock: true}),
  ...
  const handleWheel = useCallback(
    (event: WheelEvent | TouchEvent) => {
      ...
      if (
        Math.abs(
          (floatingRef.current?.offsetHeight ?? 0) -
    ...
    [increaseHeight, floatingRef, forceUpdate]
  );
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    if (open && controlledScrolling && floating) {
      ...
      if (item && prevActiveIndex != null) {
        const prevItemRect =
          listItemsRef.current[prevActiveIndex]?.getBoundingClientRect();
        const itemRect = item.getBoundingClientRect();
        const itemDiff = Math.abs(
          itemRect.bottom - (prevItemRect?.bottom ?? 0)
        );

        const floatingHeight = floating.offsetHeight;
        const top = item.offsetTop;
        const bottom = top + itemDiff;

        if (top < floating.scrollTop + itemDiff) {
          floating.scrollTop -= itemDiff;

          if (activeIndex != selectedIndex && activeIndex != null) {
            increaseHeight(floating, -itemDiff);
          }
        } else if (bottom + itemDiff > floatingHeight + floating.scrollTop) {
          floating.scrollTop += itemDiff;
